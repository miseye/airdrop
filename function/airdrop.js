...
const tokenInput = fs.readFileSync('./contract/coco.sol');
const tokenOutput = solc.compile(tokenInput.toString());
const tokenAbi = JSON.parse(tokenOutput.contracts[':COCO'].interface);
const addresses = require('./addrs').addrs;
const amounts = require('./amounts').amounts;
...
function transfer(erc20TokenContractAddress , airdropDestinationAddresses, airdropAmounts,hashIdCallBack,successCall, errorCall, nonceadd) {
    ...
    execute.executeFunction(airContractAddress,dataAbi,hashIdCallBack,successCall,errorCall, nonceadd);
}
...
//每次空投数量
const onceAmountOfAirdropList = 50;
...
function transferWithAddressAndAmounts(addresses,amounts,resultCall) {
    ...
    for (let i in amounts){
        ...
        let obj = web3.utils.toWei(amount, 'wei');
    ...
    for(i=3;i<Math.floor(totalAmountOfAirdropList/onceAmountOfAirdropList);i++){
        startHandleAirdrop(i);
    }
}
...
function startHandleAirdrop(index) {
    ...
    transfer(tokenContractAddress,currentAddresses,currentAmounts,function (hashId) {
...
transferWithAddressAndAmounts(addresses,amounts);
module.exports = {
    startAirdrop,
    sendLastPartAirdrop
};
